...
const router = (fastify, { }, next) => {
	...
	fastify.post('/queue/prepare/data', { preHandler: [fastify.authenticate] }, async (req: fastify.Request, reply: fastify.Reply) => {
		const queueId: any = req.body.queueId;
		const printSmallQueue = req.body.printSmallQueue || 'N';
		...
		if (queueId) {
			try {
				...
				if (rs[0].length) {
					...
					reply.status(HttpStatus.OK).send({ statusCode: HttpStatus.OK, data });
				} else {
					reply.status(HttpStatus.OK).send({ statusCode: HttpStatus.BAD_REQUEST, message: 'ไม่พบรหัสคิวที่ต้องการ' })
				}
